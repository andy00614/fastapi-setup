from typing import List, Optional
from sqlalchemy import select, update, delete
from sqlalchemy.ext.asyncio import AsyncSession
from app.models.post import Post
from app.schemas.post import PostCreate, PostUpdate, PostOut

class PostDBRepo:
    def __init__(self, session: AsyncSession) -> None:
        self.session = session  # 像把 Prisma/TypeORM 的 manager 注入进来

    async def list(self) -> List[PostOut]:
        stmt = select(Post).order_by(Post.id)
        res = await self.session.execute(stmt)
        rows = res.scalars().all()
        # 把 ORM 实例转成 Pydantic 响应模型（也可以直接返回 ORM，再让 FastAPI 用 from_attributes=True）
        return [PostOut(id=r.id, title=r.title, content=r.content) for r in rows]

    async def get(self, post_id: int) -> Optional[PostOut]:
        row = await self.session.get(Post, post_id)
        if not row:
            return None
        return PostOut(id=row.id, title=row.title, content=row.content)

    async def create(self, payload: PostCreate) -> PostOut:
        row = Post(**payload.model_dump())
        self.session.add(row)
        await self.session.commit()     # 提交事务
        await self.session.refresh(row) # 刷新以拿到自增 id 等
        return PostOut(id=row.id, title=row.title, content=row.content)

    async def update(self, post_id: int, payload: PostUpdate) -> Optional[PostOut]:
        patch = payload.model_dump(exclude_unset=True)
        if not patch:
            # 没有任何可更新字段，直接返回当前数据（或按需报错）
            current = await self.session.get(Post, post_id)
            if not current:
                return None
            return PostOut(id=current.id, title=current.title, content=current.content)

        stmt = (
            update(Post)
            .where(Post.id == post_id)
            .values(**patch)
            .returning(Post.id, Post.title, Post.content)
        )
        res = await self.session.execute(stmt)
        row = res.first()
        if not row:
            await self.session.rollback()
            return None
        await self.session.commit()
        rid, title, content = row
        return PostOut(id=rid, title=title, content=content)

    async def delete(self, post_id: int) -> bool:
        stmt = delete(Post).where(Post.id == post_id)
        res = await self.session.execute(stmt)
        await self.session.commit()
        return res.rowcount > 0
